...
public abstract class ScheduleWithClusterPriority {
    ...
    public ScheduleWithClusterPriority(ResourceGetter resourceGetter,
    ...
    /**
     * Tries to schedule a ScheduleJob in a given time frame (start to end exclusive) with given
     * priority. Comparator is used to sort the faculty resources in a day, so that they can
     * be considered in a specific order.

     * @param job job to be scheduled
     * @param start first day that should be considered
     * @param end first day that should NOT be considered
     * @param comp comparator that enforces a specific order of resources
     * @return a list of ScheduledInstances representing a ScheduleJob that can be saved
     *     to a database.
     */
    @SuppressWarnings("PMD")
    public List<ScheduledInstance> scheduleBetween(ScheduleJob job, LocalDate start,
                                                   LocalDate end, Comparator<FacultyResource> comp) {
